...
#if defined UARTD_USE_TX && !defined UARTD_TX_IO
  #error No UARTD TX IO defined!
#endif
#if defined UARTD_USE_RX && !defined UARTD_RX_IO
...
void uartd_getbuf(uint8_t* buf, uint16_t len)
{
uint16_t i;

  for (i = 0; i < len; i++) buf[i] = uartd_getc();
}
