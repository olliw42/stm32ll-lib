...
#include "stdstm32-subghz.h"
  ...
  #define SPIC_MOSI_IO             IO_PB15 // IO_PC2 ?
    ...
    // H5: but PB13 is a UCPD IO with no fast driver, SCK on it is specd for 3 MHz max, above that use SPIC_USE_SCK_IO = IO_PB10, also AF5
  #endif
#endif
#ifdef SPIC_USE_SPI3
  ...
  #define SPIC_MOSI_IO             IO_PB5
    ...
    #undef SPIC_MOSI_IO
    #undef SPIC_IO_AF
    #define SPIC_SCK_IO            IO_PC10
    #define SPIC_MISO_IO           IO_PC11
    #define SPIC_MOSI_IO           IO_PC12
    #define SPIC_IO_AF             IO_AF_6
  #elif defined STM32H5
...
void spic_disable(void)
{
  ...
  LL_SPI_SuspendMasterTransfer(SPIC_SPIx);
  while (!LL_SPI_IsActiveFlag_SUSP(SPIC_SPIx)) {};
...
{
#if defined STM32H5
  // SPI v2: continuous master mode (set in spic_init); poll TXP/RXP
  while (!LL_SPI_IsActiveFlag_TXP(SPIC_SPIx)) {};
  LL_SPI_TransmitData8(SPIC_SPIx, c);
  while (!LL_SPI_IsActiveFlag_RXP(SPIC_SPIx)) {};
  return LL_SPI_ReceiveData8(SPIC_SPIx);
#else
#ifdef SPIC_USE_SUBGHZSPI
  while (!LL_SPI_IsActiveFlag_TXE(SPIC_SPIx)) {}; // we don't do that originally, but it's suggested by cubemx
#endif
  LL_SPI_TransmitData8(SPIC_SPIx, c);
  while (!LL_SPI_IsActiveFlag_RXNE(SPIC_SPIx)) {};
  return LL_SPI_ReceiveData8(SPIC_SPIx);
...
{
  return spic_transmitchar(spic_nop_byte);
}
...
{
  return (((uint16_t)spic_transmitchar(spic_nop_byte)) << 8) + spic_transmitchar(spic_nop_byte);
}
...
void spic_setmode(SPIMODEENUM mode)
{
  ...
  LL_SPI_StartMasterTransfer(SPIC_SPIx);
#else
  while (!LL_SPI_IsActiveFlag_TXE(SPIC_SPIx)) {}
...
LL_SPI_InitTypeDef SPI_InitStruct = {};
  ...
  spic_deselect(); // deselect SPI: CS high
#endif
  ...
  LL_SPI_SetTransferSize(SPIC_SPIx, 0);
#endif
  ...
  // gate per-byte transmit/receive in spic_transmitchar
  LL_SPI_StartMasterTransfer(SPIC_SPIx);
#else
  // Empty SPIx
  while (!LL_SPI_IsActiveFlag_TXE(SPIC_SPIx)) {}
