...
#include "stdstm32-subghz.h"
  ...
  #define SPI_MOSI_IO             IO_PB15 // IO_PC2 ?
    ...
    // H5: but PB13 is a UCPD IO with no fast driver, SCK on it is specd for 3 MHz max, above that use SPI_USE_SCK_IO = IO_PB10, also AF5
  #endif
#endif
#ifdef SPI_USE_SPI3
  ...
  #define SPI_MOSI_IO             IO_PB5
    ...
    #undef SPI_MOSI_IO
    #undef SPI_IO_AF
    #define SPI_SCK_IO            IO_PC10
    #define SPI_MISO_IO           IO_PC11
    #define SPI_MOSI_IO           IO_PC12
    #define SPI_IO_AF             IO_AF_6
  #elif defined STM32H5
...
void spi_disable(void)
{
  ...
  LL_SPI_SuspendMasterTransfer(SPI_SPIx);
  while (!LL_SPI_IsActiveFlag_SUSP(SPI_SPIx)) {};
...
{
#if defined STM32H5
  // SPI v2: continuous master mode (set in spi_init); poll TXP/RXP
  while (!LL_SPI_IsActiveFlag_TXP(SPI_SPIx)) {};
  LL_SPI_TransmitData8(SPI_SPIx, c);
  while (!LL_SPI_IsActiveFlag_RXP(SPI_SPIx)) {};
  return LL_SPI_ReceiveData8(SPI_SPIx);
#else
#ifdef SPI_USE_SUBGHZSPI
  while (!LL_SPI_IsActiveFlag_TXE(SPI_SPIx)) {}; // we don't do that originally, but it's suggested by cubemx
#endif
  LL_SPI_TransmitData8(SPI_SPIx, c);
  while (!LL_SPI_IsActiveFlag_RXNE(SPI_SPIx)) {};
  return LL_SPI_ReceiveData8(SPI_SPIx);
...
void spi_setmode(SPIMODEENUM mode)
{
  ...
  LL_SPI_StartMasterTransfer(SPI_SPIx);
#else
  while (!LL_SPI_IsActiveFlag_TXE(SPI_SPIx)) {}
...
LL_SPI_InitTypeDef SPI_InitStruct = {};
  ...
  SPI_InitStruct.ClockPolarity = LL_SPI_POLARITY_LOW;
  SPI_InitStruct.ClockPhase = LL_SPI_PHASE_1EDGE;
#if defined SPI_USE_CLOCKSPEED_24MHZ
  SPI_InitStruct.BaudRate = SUBGHZSPI_BAUDRATEPRESCALER_2;
#elif defined SPI_USE_CLOCKSPEED_12MHZ
  SPI_InitStruct.BaudRate = SUBGHZSPI_BAUDRATEPRESCALER_4;
#elif defined SPI_USE_CLOCKSPEED_6MHZ
  ...
  LL_SPI_SetTransferSize(SPI_SPIx, 0);
#endif
  ...
  // gate per-byte transmit/receive in spi_transmitchar
  LL_SPI_StartMasterTransfer(SPI_SPIx);
#else
  // Empty SPIx
  while (!LL_SPI_IsActiveFlag_TXE(SPI_SPIx)) {}
